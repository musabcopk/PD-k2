...
const initialAssets: Asset[] = [
  { id: 1, name: "Habib Bank Limited", symbol: "HBL", quantity: 200, avgPrice: 140, currentPrice: 145.50, value: 29100, change: 3.93 },
  { id: 2, name: "Pakistan Petroleum", symbol: "PPL", quantity: 500, avgPrice: 80, currentPrice: 58.93, value: 29465, change: -26.34 },
]

export default function PKRAssetsPage() {
      ...
      <main className="flex-1 p-6 ml-64">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-2xl font-bold text-foreground">PKR Assets</h1>
              <p className="text-muted-foreground">Manage your Pakistani stock portfolio</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-card border border-border rounded-xl p-4">
              ...
              <p className="text-2xl font-bold text-foreground mt-1">Rs {totalValue.toLocaleString()}</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-4">
              <p className="text-xs text-muted-foreground uppercase tracking-wider">Total Invested</p>
              <p className="text-2xl font-bold text-foreground mt-1">Rs {totalInvested.toLocaleString()}</p>
            </div>
            <div className="bg-card border border-border rounded-xl p-4">
              ...
              <tbody>
                {assets.map((asset) => (
                  <tr key={asset.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="text-right px-6 py-4 text-foreground">{asset.quantity.toLocaleString()}</td>
                    <td className="text-right px-6 py-4 text-foreground">Rs {asset.avgPrice.toLocaleString()}</td>
                    <td className="text-right px-6 py-4 text-foreground">Rs {asset.currentPrice.toLocaleString()}</td>
                    <td className="text-right px-6 py-4 text-foreground font-medium">Rs {asset.value.toLocaleString()}</td>
